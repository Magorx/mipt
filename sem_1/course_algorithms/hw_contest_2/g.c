...
Ведём две кучи - мин и макс
При пуше пушим в обе
При попе из, например, минимальной, по индексам нахдим элемент в куче максимумов, меняем его с последним в куче
максимумов и забываем про него, уменьшая размер на единицу. Т.к. мы могли положить элемент, который раньше был
последним, в неправильное место относительно других значений в куче, вызывем sift_up. sift_down не требуется,
так как мин. элемент точно лежит в низу макс кучи.
Для эл-тов и макс кучи поступаем зеркально аналогично
*/
int main() {
    int n = 0;
    scanf("%d", &n);
    ...
    Heap h_min = {};
    Heap h_max = {};
    Heap_construct(&h_min, n + 2, -1);
    Heap_construct(&h_max, n + 2, +1);
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        if (strcmp(str, "Insert(") == 0) {
            ...
            scanf("%lld)", &val);
            //printf("added %lld\n", val);
            Heap_push(&h_min, val, i);
            Heap_push(&h_max, val, i);
        } else if (strcmp(str, "GetMin") == 0) {
            printf("%lld\n", Heap_top(&h_min));
            int b_idx = h_min.b_indexes[1];
            Heap_pop(&h_min); // deleted from min heap

            int idx = h_max.f_indexes[b_idx];
            if (idx == h_max.size - 1) {
                h_max.size -= 1;
            } else {
                Heap_swap(&h_max, idx, h_max.size - 1);
                h_max.size -= 1;
                Heap_sift_up(&h_max, idx);
            }
        } else if (strcmp(str, "GetMax") == 0) {
            printf("%lld\n", Heap_top(&h_max));
            int b_idx = h_max.b_indexes[1];
            Heap_pop(&h_max); // deleted from min heap

            int idx = h_min.f_indexes[b_idx];
            if (idx == h_min.size - 1) {
                h_min.size -= 1;
            } else {
                Heap_swap(&h_min, idx, h_min.size - 1);
                h_min.size -= 1;
                Heap_sift_up(&h_min, idx);
            }
        }
    }
