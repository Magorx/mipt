...
// Реализуем splay с помощью splay и используем его
int main() {
	int n = 0;
	scanf("%d", &n);

	SplayTree<long long> *tree = new SplayTree<long long>;
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (type == '+') {
        	...
        	SplayTree<long long> *right = tree->split(r);
        	SplayTree<long long> *mid = tree->split(l - 1);

        	SplayTreeNode<long long> *mroot = mid->get_root();
        	y = mroot ? mroot->s : 0;
        	printf("%lld\n", y);
