    ...
    int get_link(int v) {
        if (data[v].link == -1) {
            if (!v || !data[v].parent) {
                ...
                data[v].link = transition(get_link(data[v].parent), data[v].own_letter);
            }
        }
        ...
        if (data[v].transition.find(c) == data[v].transition.end()) {
            if (data[v].to.find(c) != data[v].to.end()) {
                ...
                if (!v) {
                    ...
                    data[v].transition[c] = transition(get_link(v), c);
                }
            }
        }
